...
| 5 V, buck A | out | `carrier/mcu` through `HDR-DEV`, `carrier/led-strip-drive` | `matrix-led-current` | Through `D-USBOR` onto the dev board's 5 V pin, and on to the 74AHCT125 |
| 5 V, buck B | out | `carrier/display-and-service-uart` | — | On `J-DISP`. Buck B's location is open — see *Still open* |
| `AGND_INST` | ref | `carrier/breath-adc`, `carrier/breath-excitation-reference`, `interfaces/breath-sense-link` | `dig-gnd-topology` | **The instrument analog star, and this circuit is its origin** — which is why the row exists at all. It reaches the `PWR_GND` pour on a **single tie**, and that tie is copper rather than a part, so `netlist.yaml` declares the net with one endpoint. Everything analog on this board returns here and **not** to the pour. **This row did not exist until 2026-09-26**: the page that originates the net named it only inside the `PWR_GND` row's prose, so three circuits cited a peer that never cited them back |
| `PWR_GND` pour | ref | `carrier/display-and-service-uart`, `carrier/led-strip-drive`, `carrier/mcu` | `dig-gnd-topology` | The whole board returns here, and the aluminium key plate through `MECH-GNDBOND`, which can only originate here. **`carrier/breath-adc` and `carrier/breath-excitation-reference` are no longer listed**: both of those pages say their return is `AGND_INST`, which reaches this pour on the **single tie** and is a different node everywhere else — and that distinction is the whole point of the star |
