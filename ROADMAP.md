...
| E7 | DAC raw | Commanded codes produce expected voltages on the meter, all six channels. **Select `R-REG-SET-ADJGND` here** — and **never below AVDD = 5.00 V**: the DAC8568's **C grade is specified only for 5.0–5.5 V** (SBAS430E, `datasheets/analog/DAC8568CIPW.pdf`), and this step otherwise optimises for top-code compression across a 0.66 V spread with no floor. Added 2026-09-21; the step had no lower bound before |
