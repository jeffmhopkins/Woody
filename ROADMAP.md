...
| E10 | Remaining channels | Analog breath stage: in-amp receiver with `REF` grounded, gain/offset knobs. **Set `TRIM-BREATH-ZERO` first**, until the in-amp output reads 0 V — then gain, then the panel offset, in that order (`hardware/module/breath-receive-stage.md`). **Commission with a meter on the jack, not the display** — the analog and digital representations are calibrated separately on purpose (ADR 0003). **Pull the umbilical mid-note with the mouthpiece at rest** and confirm the breath jack parks quietly: the watchdog has no authority over it by design, and this is the check that the design is right about why (ADR 0004). Four mod channels trimmed |
...
| E12 | Module PCB + panel | 8HP panel cut, module assembled and racked. etherCON braced to the PCB — good practice at 8HP rather than the structural necessity it was at 6HP. **Ground laid out to the star rule in ADR 0004** — one origin at the power inlet, `PWR_GND` and `DIG_GND` each on their own copper, `AGND` not a return at all. Free now, a bodge wire or a respin afterwards |
...
| **Key-chain error counter over an hour, LEDs and WiFi active** | E4 | The marker pattern's whole purpose. A non-zero count says the looms need work while the body is still openable (ADR 0001). **With the registers on the carrier the marker bits are board-local, so they test the chain and the firmware and NOT the loom** — an earlier version of this row claimed the opposite. Loom coverage comes from the sentinel conductors instead |
| **Do 23 conductors fit the side channel?** | M4 | The one real cost of putting the registers at the tail, and the only part of that decision taken on faith. Four ribbons, one per cluster, against the real cavity section — before the plate DXF is cut (ADR 0001, ADR 0009) |
